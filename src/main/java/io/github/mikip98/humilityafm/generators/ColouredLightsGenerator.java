...
public class ColouredLightsGenerator {
    ...
    public static void init() {
        for (byte i = 0; i < GenerationData.vanillaColorPallet.length; ++i) {
            colouredTorchWeakVariants[i] = new TorchBlock(FabricBlockSettings.create().luminance(7), ParticleTypes.FLAME);
            colouredTorchVariants[i] = new TorchBlock(FabricBlockSettings.create().luminance(11), ParticleTypes.FLAME);
            colouredTorchStrongVariants[i] = new TorchBlock(FabricBlockSettings.create().luminance(15), ParticleTypes.FLAME);
