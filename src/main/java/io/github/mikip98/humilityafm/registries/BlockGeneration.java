    ...
    protected static Block[] generateWoodenMosaicVariants() {
        ...
        for (BlockMaterial variantMaterial : ActiveGenerationData.woodenMosaicVariantMaterials) {
            ...
            if (isFirstWoodFireproof && isSecondWoodFireproof) {
                block = registerWithItem("wooden_mosaic_" + variantName, Block::new, fireproofWoodenMosaicSettings);
            } else {
                block = registerWithItem("wooden_mosaic_" + variantName, Block::new, burnableWoodenMosaicSettings);
                if (isFirstWoodFireproof || isSecondWoodFireproof) {
        ...
        if (!ModConfig.getEnableCandlestickBeta()) {
            ...
            for (int i = 0; i < metalSetCount; i++) {
                ...
                for (BlockMaterial material : ActiveGenerationData.rustingCandlestickMaterials[i]) {
                    metalSetWallVaraintList.add((RustableCandlestick) register("candlestick_wall_" + material.getSafeName(), RustableCandlestick::new, RustableCandlestick.defaultSettings));
                    metalSetFloorVariantList.add((FloorRustableCandlestick) register("candlestick_" + material.getSafeName(), FloorRustableCandlestick::new, RustableCandlestick.defaultSettings));
                }
